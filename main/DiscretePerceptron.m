...
for LOOCV=1:520
    ...
    teTr = Tr_all(:,LOOCV);
    Tr_all(:,LOOCV) = [];
    net = perceptron('hardlims', 'learnp');
    net.performFcn = 'sse';
    ...
    accuracy_train = accuracy_train + true_/length(Tr_all(1,:));   
end
...
for i=1:length(targets(1,:))
    ...
    if netResult_noise2(:,i) == targets(:,i)
       accuracy_n25 = accuracy_n25 +1;
    end
end
accuracy_noisy15 = accuracy_n15/length(targets(1,:)) ;
accuracy_noisy25 = accuracy_n25/length(targets(1,:));
msg = sprintf(['Results of discrete perceptron network:\n\n',...
               'train accuracy = %.4g\n',...
               'test accuracy = %.4f\n',...
